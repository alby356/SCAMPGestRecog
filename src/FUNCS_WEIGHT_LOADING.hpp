...
void load_filter_weights_into_grid_F(int filters,int filter_size,const void *filter_weights_pointer, int grid_cell_size,int grid_posx, int grid_posy, int grid_sizex,int grid_padding)
{
	...
	for(int n = 0 ; n < filters; n++)
	{
		...
		int offy = grid_posy+gridy*(grid_cell_size+grid_padding)-1;//IMPORTANT -1 ???

		MISC_FUNCTIONS::load_centered_rect_into_DREG(DENUM::R1, offx,offy, grid_cell_size, grid_cell_size);

		for(int i = 0 ; i < filter_size*filter_size; i++)
		{
			scamp5_load_pattern(R2,(offy-i/filter_size),(offx+4-i%filter_size),252,252);
			scamp5_kernel_begin();
				AND(R3,R1,R2);
			scamp5_kernel_end();
