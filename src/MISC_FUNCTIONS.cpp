...
// ---------------------------------------------------------------------------
// REMOVED: DREG_SUM_R10(), DREG_SUM_R10_STEVE_OPT(), and
// DREG_STACKING_R10_INTO_R1_STEVE_OPT() (originally lines 1379-1754 of the
// reference repo file this was copied from).
//
// These three functions do not compile against the SCAMP5 SDK actually
// installed here (MCUXpresso workspace, SDK 25.6.136): they call XOR(...),
// which this SDK version does not expose (only XORX, a destructive variant
// with a different side effect on its second argument -- see
// scamp5_kernel_api_macro_digital.hpp). This is a real build error, caught by
// actually compiling against the installed SDK with arm-none-eabi-c++, not a
// hypothetical one.
//
// None of these three functions are called anywhere in the gesture-recognition
// pipeline (GESTURE_CAPTURE.cpp / MAIN_GESTURE_SINGLE_LAYER_16.cpp only use
// shift_R11 / load_rect_into_DREG / load_DREG_into_F from this file), so they
// are dead code for this project. Removed rather than XOR->XORX-renamed: XORX
// clobbers its Rx argument as a side effect, and since these functions are
// never exercised, that behavior change cannot be verified either way.
// ---------------------------------------------------------------------------
